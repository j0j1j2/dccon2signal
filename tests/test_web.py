...
async def test_emoji_catalog_and_picker(app):
    ...
    assert "class=sticker-title" in detail.text
    assert ".sticker>div" not in detail.text
...
async def test_home_uses_stickergen_mobile_tabs(app):
    ...
    assert "<h1 class=wordmark>STICKERGEN</h1>" in response.text
    assert "<span class=eyebrow>STICKERGEN</span>" not in response.text
